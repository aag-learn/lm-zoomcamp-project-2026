...
- `podman-compose` reads `.env` from the working directory and its values win over already-`export`ed shell environment variables of the same name — the reverse of Docker Compose's documented precedence (shell env > `.env` file). Confirmed directly: exporting `POSTGRES_PORT=15432` etc. before `podman-compose -p <other-project> up` had no effect; the committed `.env`'s `POSTGRES_PORT=5432` won, causing a port collision with the real stack. To run an isolated second project with different ports, temporarily edit `.env` itself (back it up first, restore it byte-identical afterward) rather than relying on shell exports.
