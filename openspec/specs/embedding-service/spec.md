...
Provide a containerized Python HTTP service that serves text-embedding and cross-encoder reranking scores from locally-loaded ML models, independent of and callable by any future Rails service. Both models load once at process startup, are exposed via a small FastAPI application (`POST /embed`, `POST /rerank`, `GET /health`), and are reachable from the host for direct `curl`-based testing. This service is scoped to ML serving only and has no `ansible-core`/`ansible-doc` responsibilities.
